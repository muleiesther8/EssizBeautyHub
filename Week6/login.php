<?php
// ============================================================
// ESSIZ BEAUTY HUB — Week 6 Login
// BIT3208 Advanced Web Design and Development
// NEW: Brute Force Protection — Login Attempt Limiting
// ============================================================
session_start();
...
if (isset($_GET['updated']))    $success = 'Profile updated successfully!';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($attempt_check['locked']) {
            $error = "⛔ Account temporarily locked. Try again in {$attempt_check['minutes']} minute(s).";
        } else {
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                $remaining = max(0, 3 - ($attempt_check['attempts'] + 1));
                $error = $remaining > 0
...
<div class="auth-wrapper">
  <div class="auth-card fade-in">
    ...
    <form id="loginForm" action="login.php" method="POST" novalidate>
      ...
      <div class="demo-hint">
        <p>🔑 Admin: <strong>admin@essizbeautyhub.com</strong> / password</p>
  ...
  <div class="auth-side fade-in">
    <div class="auth-side-content">
      ...
      <div style="margin-top:32px;padding:16px;background:rgba(255,255,255,0.15);border-radius:12px;">
        <p style="font-size:13px;color:rgba(255,255,255,0.9);">🔐 Secured with bcrypt password hashing and brute force protection</p>
      </div>
    </div>
  </div>
</div>
